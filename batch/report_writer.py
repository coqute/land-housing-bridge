...
def _build_detail_blocks(lh_result: dict | None, ih_result: dict | None) -> list[dict]:
    """신규 공고 목록을 bulleted_list_item 블록으로 생성.
    ...
    # LH 신규 공고
    lh_new = (lh_result or {}).get("new_notices", [])
    if lh_new:
        blocks.append({
            "type": "heading_3",
            "heading_3": {"rich_text": rich_text(f"LH 신규 공고 ({len(lh_new)}건)")},
        })
        for n in lh_new[:_MAX_DETAIL_ITEMS]:
            blocks.append({
                "type": "bulleted_list_item",
                "bulleted_list_item": {
                    "rich_text": rich_text(f"{n.get('PAN_NM', '')} (ID: {n.get('PAN_ID', '')})"),
                },
            })
        if len(lh_new) > _MAX_DETAIL_ITEMS:
            blocks.append({
                "type": "paragraph",
                "paragraph": {"rich_text": rich_text(f"... 외 {len(lh_new) - _MAX_DETAIL_ITEMS}건")},
            })

    # IH 신규 공고
    ih_new = (ih_result or {}).get("new_notices", [])
    if ih_new:
        blocks.append({
            "type": "heading_3",
            "heading_3": {"rich_text": rich_text(f"IH 신규 공고 ({len(ih_new)}건)")},
        })
        for n in ih_new[:_MAX_DETAIL_ITEMS]:
            blocks.append({
                "type": "bulleted_list_item",
                "bulleted_list_item": {
                    "rich_text": rich_text(f"{n.get('sj', '')}"),
                },
            })
        if len(ih_new) > _MAX_DETAIL_ITEMS:
            blocks.append({
                "type": "paragraph",
                "paragraph": {"rich_text": rich_text(f"... 외 {len(ih_new) - _MAX_DETAIL_ITEMS}건")},
            })
