...
- `_gather_all_lh_notices(days, tp_codes, **kwargs)` — 인천(CNP_CD=28) + 전국(CNP_CD 없음) 이중 조회 → 전국 결과는 `filter_region_relevant()`로 인천 관련+전국 대상만 필터 → `dedup_by_pan_id()` 병합. 인천 조회 우선.
